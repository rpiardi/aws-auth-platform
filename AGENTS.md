...
Create a basic GitLab CI pipeline for OpenTofu deployment using the official
...
- configure `root_dir` as `terraform`;
- configure `state_name` as `auth-platform-prd`;
- provide the standard OpenTofu formatting, validation, planning, and apply jobs;
- publish the OpenTofu plan artifact through the component defaults;
- keep `apply` as a manual deployment step.

The `apply` job should:

- be manual;
- run only from the default deployment branch, preferably `main`;
- reuse the plan artifact generated by the `plan` job.

Disable destructive cleanup jobs initially:
...
Keep the pipeline minimal and operational.
...
- GitLab CI pipeline overview using OpenTofu
...
The only CI/CD expected initially is the basic GitLab CI OpenTofu pipeline described above.
